...
const SubBar = ({state, toggleTweets, setCategory, toggleDropdown, onZipChange, fetchStores}) => {
    return (
        <div>
            ...
            <nav className='pa1 f6' style={{alignItems: "center", justifyItems : "center",color: "white",backgroundColor: "#001b44F1",display: 'grid', gridTemplateColumns:"0.22fr repeat(12, auto)"}}>
                ...
                {/* {state.dropdown && (
                    <div>
                         <NearMe toggleDropdown={toggleDropdown} fetchStores={fetchStores} onZipChange={onZipChange}/>
                          </div>)} */}
                
                <text className='ml2 pointer dim near-white' onClick={() => fetchStores()}>Stores Near Me</text>
                ...
                <text className='ml2 pointer dim  near-white' onClick={() => setCategory(true, 'baby care')}>Baby Products</text>
                <text className='ml2 pointer dim  near-white' onClick={() => toggleTweets()}>Show Tweets</text>
